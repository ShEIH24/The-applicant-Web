...
load_dotenv(encoding="utf-8")  # читаем .env ДО всех остальных импортов
...
async def _wait_for_db() -> None:
    """
    Ждёт, пока MySQL примет соединения.
    MySQL при первом запуске перезапускается дважды (инициализация схемы),
    поэтому healthcheck Docker может дать false-positive — страхуемся здесь.
    """
...
async def _ensure_admin_exists() -> None:
    """
    При первом запуске автоматически создаёт администратора
    из переменных окружения ADMIN_USERNAME / ADMIN_PASSWORD / ADMIN_FULLNAME.
    Если хотя бы один пользователь уже есть — ничего не делает.
    """
...
async def lifespan(app: FastAPI):
    # 1. Ждём готовности БД (защита от false-positive healthcheck при первом запуске)
    await _wait_for_db()
    ...
    # 3. Создаём первого администратора если нужно
...
async def health_check():
    """Проверка работоспособности для Docker healthcheck и мониторинга"""
    return {"status": "ok"}
